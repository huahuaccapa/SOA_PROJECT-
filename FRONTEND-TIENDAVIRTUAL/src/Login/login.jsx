...
import { Mail, Lock, Eye, EyeOff, LogIn, Cpu, Sparkles } from 'lucide-react';
...
const Login = () => {
  ...
  const MOCK_USERS = {
    'admin@techstore.com': {
      password: 'admin123',
      user: { id: 1, email: 'admin@techstore.com', fullName: 'Administrador TechStore', role: 'ADMIN' }
    },
    'usuario@techstore.com': {
      password: 'user123',
      user: { id: 2, email: 'usuario@techstore.com', fullName: 'Usuario Demo', role: 'USER' }
    },
    'maria@email.com': {
      password: 'maria123',
      user: { id: 3, email: 'maria@email.com', fullName: 'María García', role: 'USER' }
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••"
                required
                className={`w-full pl-12 pr-12 py-3.5 bg-base-primary border rounded-xl text-text-base text-sm focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all ${
                  error ? 'border-red-400' : 'border-border-color'
                }`}
              />
              <button
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center cursor-pointer">
              ...
              />
              <div className="w-5 h-5 bg-base-primary border border-border-color rounded mr-2.5 peer-checked:bg-primary peer-checked:border-primary relative peer-checked:after:absolute peer-checked:after:left-1.5 peer-checked:after:top-0.5 peer-checked:after:w-1.5 peer-checked:after:h-2.5 peer-checked:after:border-white peer-checked:after:border-r-2 peer-checked:after:border-b-2 peer-checked:after:rotate-45"></div>
              <span className="text-text-muted text-sm">Recordarme</span>
            </label>
            <Link to="/forgot-password" className="text-primary text-sm font-medium hover:text-primary-light transition-colors">
          ...
          <div className="text-center pt-2 border-t border-border-color">
            <p className="text-text-muted text-xs">
              <span className="font-medium">Usuarios de prueba:</span><br/>
              <span className="text-primary/70">admin@techstore.com</span> / admin123 → ADMIN<br/>
              <span className="text-primary/70">usuario@techstore.com</span> / user123 → USUARIO
            </p>
          </div>
        </form>
      </div>
